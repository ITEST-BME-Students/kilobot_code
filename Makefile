...
.PHONY: docs synced_flashing
KILOLIB = build/kilolib.a
synced_flashing: build/synced_flashing.elf build/synced_flashing.hex build/synced_flashing.lss
...
build/synced_flashing.elf: synced_flashing.c $(KILOLIB) | build
	$(CC) $(CFLAGS) -o $@ $< $(KILOLIB) 
